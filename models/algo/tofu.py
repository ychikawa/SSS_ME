...
PRUNE_LAYERS = 6  # layers < PRUNE_LAYERS use pruned merging; >= use MLERP
...
def merge_wavg(merge, prune, x, size=None, layer=0):

    if layer < PRUNE_LAYERS:
        x = prune(x)
        size = prune(size)
    else:
        # ---------- MLERP merging ----------
        x_norm = x.norm(dim=-1, keepdim=True)           # (n, seq, 1)
        x_avg  = merge(x,      mode="mean")             # (n, reduced_seq, c)
        max_norm = merge(x_norm, mode="amax")           # (n, reduced_seq, 1)

        avg_norm = x_avg.norm(dim=-1, keepdim=True).clamp(min=1e-6)
        x = (x_avg / avg_norm) * max_norm               # MLERP Eq.10
...
class CustomBlock(nn.Module):
    ...
    def forward(self, x, prefix):
        ...
        if r > 0:
            merge, prune = bipartite_soft_matching(metric, r)
            x, size = merge_wavg(merge, prune, x, size, layer=self.layer)
